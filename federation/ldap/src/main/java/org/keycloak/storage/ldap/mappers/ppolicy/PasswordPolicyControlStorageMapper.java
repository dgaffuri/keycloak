...
public class PasswordPolicyControlStorageMapper extends AbstractLDAPStorageMapper implements PasswordUpdateCallback {
    ...
    @Override
    public LDAPOperationDecorator beforePasswordUpdate(final UserModel user, final LDAPObject ldapUser, final UserCredentialModel password) {
    ...
    @Override
    public void passwordUpdated(final UserModel user, final LDAPObject ldapUser, final UserCredentialModel password) {
    }

    @Override
    public void passwordUpdateFailed(final UserModel user, final LDAPObject ldapUser, final UserCredentialModel password, final ModelException exception) {
    ...
    public boolean onAuthenticationFailure(final LDAPObject ldapUser, final UserModel user, final AuthenticationException ldapException, final RealmModel realm) {
        ...
        if (response != null && response.getError() != null) {
        	...
        	case passwordExpired:
                ...
                if (!user.getRequiredActionsStream().anyMatch(a -> UserModel.RequiredAction.UPDATE_PASSWORD.name().equals(a))) {
                    user.addRequiredAction(UserModel.RequiredAction.UPDATE_PASSWORD);
                }
                return true;
        	case accountLocked:
        		// returned only if ppolicy_use_lockout is explicitly set in OpenLDAP ppolicy overlay configuration at database level
